...
app.controller('SolicitudCtrl', function ($scope, $http) {
    ...
    $scope.update = function (banda_id, usuario_id, estado, num_integrantes) {
        $http({
            method: 'POST',
            url: $scope.api_server + 'editar/solicitud_banda/'+banda_id+'/'+usuario_id,
            data: {
                estado: estado,
            ...
            if(response.data.message === 'Evento llenó su cupo'){
                ...
                setTimeout(function () {
                    window.location.href = $scope.api_server+'solicitudes_bandas';
                }, 2000);
            } else {
                toastr.success(response.data.message);
                setTimeout(function () {
